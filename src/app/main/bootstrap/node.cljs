;; The below code is directly lifted and modified from bootstrapping
;; stage of shadow-cljs in browser.cljs at
;; https://github.com/thheller/shadow-cljs
;; Copyright Thomas Heller

(ns app.main.bootstrap.node
...
(defonce init-opts (atom {:path "/bootstrap" :load-on-init []}))
...
(defn transit-load [path callback]
  (io/file-reader path (fn[err data] (callback (transit-read data)))))
...
(defn execute-load! [compile-state-ref {:keys [type text uri ns provides] :as load-info}]
  ...
  (case type
    :analyzer
    (let [data (transit-read (js->clj text))]
      (println data)
    ...
    (if (empty? load-info)
      ...
      (let [uris
            ...
            fchan (chan)]
               ...
               (doseq [info load-info]
          (go (io/file-reader (:uri info) (fn[err data]
                                            (if err
                                            (println err)
                                            (go (>! fchan data)))))
          (go (execute-load! compile-state-ref (assoc info :text (<! fchan))))))
